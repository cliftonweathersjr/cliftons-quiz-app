...
In this project, I will be building a React app (quiz app) using AWS Amplify and Cognito. I will also be setting up continuous integration and continuous deployment (CI/CD) using GitHub.
...
To start this step, create a directory for the application and change the current directory to that folder. From there, run the command `npm install -g @aws-amplify/cli`. This will install the Amplify CLI. Afterward, run `amplify configure`. This will return a pop-up for you to sign in to the AWS console. While in the console, an IAM user will need to be created for the Amplify CLI. To do such, navigate to the IAM's User section in the console. Once there, create a user with the name `amplify-dev`. When setting permissions, instead of adding the user to a group, attach policies directly. Under the permissions policies, search for the policy name `AdministratorAccess-Amplify`. Once selected, click next and then create user. After creating the user, an Access Key will be needed for the IDE to use when running commands. To create this, navigate to the security credentials section for the newly created user. The Access Key section will have the option to create an access key, select it. The use for this key will be the Command Line Interface because it will be used in the terminal of the IDE. Once that is selected and the confirmation at the bottom is checked, click next and then create an access key. Take note of the access key information because the credentials will be needed by the terminal. The IDE provides a link with detailed instructions if needed.

After creating the user and its access keys, go back to the terminal and hit enter. The IDE will ask for the current region and after hitting two more times, a request for the Access Key information will be given. Enter the information as it is requested. The last part of this step will be giving the profile a name, the name should be `amplify-dev-local`.

## Step Two: Create React App and Initialize the Amplify Project

To create the React app, run the command `npx create-react-app <name of your app>`. This will create a folder inside of the current directory with the name of the app. Once the command is finished, change the directory to that folder to begin initializing the Amplify project.
...
Once selecting `yes` for the configuration. Select `AWS Profile` for the authentication method and `amplify-dev-lcoal` for the profile. Lastly, select `no` for the sharing of non-sensitive configuration information.

## Step Three: Setting Up Cognito Authentication

Run the command `amplify add auth`. After running the command, answer the following questions with the answers below.
```
Do you want to use the default authentication and security configuration? Default Configuration
How do you want users to be able to sign in? Email
Do you want to configure advanced settings? No, I am done.
```
After this is complete, run the commands `amplify push` and `yes` to continue. These commands will push everything out to AWS for the backend environment. The next thing that needs to happen is to give the Cognito, an UI to enforce authentication. Accomplish this with the following steps.
...
```
After that is complete, run the command `npm install aws-amplify @aws-amplify/ui-react` in your IDE terminal. This will install the Amplify libraries that give the login interface. Once this is complete, the application can now be run using the command `npm start`. The website can be accessed using `localhost:3000`. From there, you can create an account and test the functionality of the Cognito service by checking the user pool.
...
const quizData = [
  ...
  ```
That will be it for new files. However, there is a commented-out code in the App.js file that will need to be revised. Return that file and ensure the following lines read as shown:
```
...
To create this CI/CD, a new repository is going to be needed. Once that new repository is created and named, return to the IDE to push the local files/code to the repository. To do this follow the list of commands:
```
git init
git add .
...
The purpose of this setup using AWS Amplify and Github is to create a pipeline that allows for the deployment of changes in the file to be automated. To test this, you can edit the `quizData.js` file (remove a question) residing in your GitHub repository and commit the changes. Once the commit has been pushed, you can check the Amplify console and notice that it has already begun the redeployment to include the new changes. After finishing, the changes should be present with a refresh.
